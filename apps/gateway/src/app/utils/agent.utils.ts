...
export class AgentUtils {
    static extractResponseContent(agentResponse: any): ExtractedContent {
        ...
        if (agentResponse.messages.length >= 2) {
            ...
            if (
                secondLastMessage?.name?.includes("RAG") ||
                secondLastMessage?.name?.includes("retrieve")
            ) {
        ...
        for (const match of responseText.matchAll(citationPattern)) {
            ...
            for (const num of numbers) {
                ...
                if (!seenRefs.has(uniqueKey)) {
                    ...
                    if (ragResponse) {
                        const textContent = this.extractTextForReference(
                            ragResponse,
                            documentName,
                            refType,
                            num
    ...
    private static extractTextForReference(
        ...
        let textStart = 0;
        const textEnd = targetCitation.index;
