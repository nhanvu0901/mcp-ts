...
export class ExtendedMongoDBChatHistory extends MongoDBChatMessageHistory {
    ...
    private async initializeSession(sessionId: string): Promise<void> {
        try {
            ...
            if (!existingSession) {
                ...
                console.log(
                    `Found existing session: ${sessionId} for user: ${this.metadata.user_id}`
                );
            }
        } catch (error) {
    ...
    override async addMessage(message: BaseMessage): Promise<void> {
        ...
        try {
            ...
            if (result.matchedCount === 0) {
                throw new Error(
                    `Session not found: ${sessionId} for user: ${this.metadata.user_id}`
                );
            }

            console.log(
                `Added message to session: ${sessionId} for user: ${this.metadata.user_id}`
            );
        } catch (error) {
    ...
    override async getMessages(): Promise<BaseMessage[]> {
        ...
        try {
            ...
            const session = await this.mongoCollection.findOne(query, {
                projection: { messages: 1, SessionId: 1, user_id: 1 },
            });

            if (!session) {
                console.log(
                    `No session found for sessionId: ${sessionId}, user_id: ${this.metadata.user_id}`
                );
                return [];
            }
            ...
            if (!session.messages || session.messages.length === 0) {
                console.log(
                    `Session found but no messages for sessionId: ${sessionId}, user_id: ${this.metadata.user_id}`
                );
                return [];
            }

            console.log(
                `Found ${session.messages.length} messages for sessionId: ${sessionId}, user_id: ${this.metadata.user_id}`
            );
    ...
    async setTitleFromMessage(message: string, maxLength = 50): Promise<void> {
        await this.initializationPromise;
        ...
        try {
            ...
            if (
                ...
                const cleanMessage = message
                    .replace(
                        /^(User ID:|Collection ID:|Document ID:|Has Document Context:).*?\n\n/g,
                        ""
                    )
                    .replace(
                        /^(Document-specific query:|General query:|Collection-specific query:)\s*/i,
                        ""
                    )
                    .trim();
                ...
                if (title && title !== "...") {
                    ...
                    await this.mongoCollection.updateOne(query, {
                        $set: {
                            title: title,
                            UpdatedAt: new Date(),
                        },
                    ...
                    console.log(
                        `Set title "${title}" for session: ${sessionId}, user: ${this.metadata.user_id}`
                    );
                }
            }
        } catch (error) {
...
export class ChatHistoryService implements IChatHistoryService {
    ...
    async buildContextMessages(allMessages: any[], sessionId: string): Promise<any[]> {
        console.log(
            `Building context messages for sessionId: ${sessionId}, total messages: ${allMessages.length}`
        );
    ...
    async cleanupDuplicateEntries(): Promise<void> {
        try {
            await this.chatMemoryCollection.deleteMany({
                $or: [{ SessionId: null }, { History: { $exists: true } }],
            });
        } catch (error) {
    ...
    async getUserConversations(userId: string, limit = 20): Promise<ConversationListItem[]> {
        try {
