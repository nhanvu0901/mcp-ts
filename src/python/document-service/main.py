...
async def create_collection(collection: CollectionCreate):
    ...
    qdrant_collection_name = f"{collection_id}_{collection.name}"
    ...
    try:
        ...
        try:
            ...
            if not qdrant_client.collection_exists(qdrant_collection_name):
                qdrant_client.create_collection(
                    ...
                    vectors_config={
                        "size": 3072,  # Azure OpenAI embedding size
                        "distance": "Cosine"
                    }
                )
        ...
        if success:
            ...
            try:
                ...
                print(f"Rollback: Deleted Qdrant collection {qdrant_collection_name}")
            except:
                pass
...
async def delete_collection(collection_id: str, user_id: str):
    try:
        ...
        qdrant_collection_name = collection_doc.get("qdrant_collection_name")
        ...
        if qdrant_collection_name:
            try:
                ...
                print(f"Warning: Failed to delete Qdrant collection {qdrant_collection_name}: {e}")
    ...
    try:
        ...
        for col in collections:
            result.append({
                ...
                "qdrant_collection_name": col.get("qdrant_collection_name")
            })
    ...
    try:
        ...
        if collection_id:
            ...
            if collection_doc and collection_doc.get("user_id") == user_id:
                qdrant_collection_name = collection_doc.get("qdrant_collection_name", f"{collection_id}_default")
        ...
        # Create temp file with proper cross-platform path
        ...
        # Check if embedding model is configured when embed=True
    ...
    try:
        ...
        if search.collection_id:
            ...
            if collection_doc and collection_doc.get("user_id") == search.user_id:
                qdrant_collection_name = collection_doc.get("qdrant_collection_name", f"{search.collection_id}_default")
            else:
                raise HTTPException(status_code=403, detail="Collection not found or not authorized")
